...
const DELETE_INVITES_BY_EVENT = gql`
  mutation DeleteInvitesByEventMutation($eventId: Int!) {
    deleteInvitesByEvent(eventId: $eventId) {
      id
    }
  }
`

const DELETE_PAIRINGS_BY_EVENT = gql`
  mutation DeletePairingsByEventMutation($eventId: Int!) {
    deletePairingsByEvent(eventId: $eventId) {
      id
    }
  }
`

const DELETE_WISH_LISTS_BY_EVENT = gql`
  mutation DeleteWishListsByEventMutation($eventId: Int!) {
    deleteWishListsByEvent(eventId: $eventId) {
      id
    }
  }
`

const DELETE_THANK_YOUS_BY_EVENT = gql`
  mutation DeleteThankYousByEventMutation($eventId: Int!) {
    deleteThankYousByEvent(eventId: $eventId) {
      id
    }
  }
`
...
const EditEventModal = ({ event, isOpen, onClose, reloadEvent }) => {
  ...
  const [deleteInvites] = useMutation(DELETE_INVITES_BY_EVENT)
  const [deleteWishLists] = useMutation(DELETE_WISH_LISTS_BY_EVENT)
  const [deletePairings] = useMutation(DELETE_PAIRINGS_BY_EVENT)
  const [deleteThankYous] = useMutation(DELETE_THANK_YOUS_BY_EVENT)

  const deleteAllEventData = async () => {
    await deleteInvites({ variables: { eventId: event.id } })
    await deletePairings({ variables: { eventId: event.id } })
    await deleteWishLists({ variables: { eventId: event.id } })
    await deleteThankYous({ variables: { eventId: event.id } })
